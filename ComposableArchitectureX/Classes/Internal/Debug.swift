    ...
    public var debugOutput: String {
        ...
        func updatePath(_ value: Any) {
            ...
            if subjectType.hasPrefix("Delay<") {
                ...
                path.append(
                    "\n.delay(for: \(Double(ns) / Double(NSEC_PER_SEC)), scheduler: \(ComposableArchitectureX.debugOutput(scheduler)))"
                )
            }
            ...
            if subjectType.hasPrefix("ReceiveOn<") {
                let scheduler = mirror.children.first(where: { label, _ in label == "scheduler" })!.value
                path.append("\n.receive(on: \(ComposableArchitectureX.debugOutput(scheduler)))")
            }
        ...
        guard mergeMany.isEmpty else {
            return
                ComposableArchitectureX
                    .debugOutput(mergeMany.filter { !ComposableArchitectureX.debugOutput($0).isEmpty })
        }
        guard empty == nil else { return "" }

        if let value = just, let transform = transform {
            let transform = withUnsafePointer(to: transform) {
                $0.withMemoryRebound(to: ((Any) -> Element).self, capacity: 1) { $0.pointee }
            }
        ...
        return """
        \(type(of: self))(\
        \(just.map { "\n\("value: \(ComposableArchitectureX.debugOutput($0))".indent(by: 2))\n" } ?? "")\
